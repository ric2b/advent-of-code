...
import Data.List (concat, find, delete, isPrefixOf, elemIndex)
...
main = do
    ...
    let departureFieldRules = filter (\FieldRule{fieldName=name} -> "departure" `isPrefixOf` name) field_rules
    print departureFieldRules

    let departureFieldIndexes = map (fromJust . (`elemIndex` fieldOrder)) departureFieldRules
    print $ product $ map (my_ticket !!) departureFieldIndexes

    -- Too low: 920453493481

    -- Arrival platform could be index 12???
    -- Type could be index 12???
